...
// Filter tanggal
var filterBulanRiwayat = "";
var filterTahunRiwayat = "";
var filterBulanGrafik = "";
var filterTahunGrafik = "";
var filterBulanKategori = "";
var filterTahunKategori = "";
...
window.addEventListener("DOMContentLoaded", function () {
    ...
    // Populate filter
    populateFilterBulan("filterBulanRiwayat");
    populateFilterBulan("filterBulanGrafik");
    populateFilterBulan("filterBulanKategori");
    populateFilterTahun("filterTahunRiwayat", transaksi);
    populateFilterTahun("filterTahunGrafik", transaksi);
    populateFilterTahun("filterTahunKategori", transaksi);
});
...
/* ============================================
   ...
   ============================================ */
function populateFilterBulan(selectId) {
    var select = document.getElementById(selectId);
    var html = '<option value="">Semua Bulan</option>';
    for (var i = 0; i < 12; i++) {
        html += '<option value="' + i + '">' + bulanNames[i] + '</option>';
    }
    select.innerHTML = html;
}

function populateFilterTahun(selectId, data) {
    var select = document.getElementById(selectId);
    var years = [];
    for (var i = 0; i < data.length; i++) {
        var tahun = new Date(data[i].tanggal).getFullYear();
        if (years.indexOf(tahun) === -1) years.push(tahun);
    }
    years.sort().reverse();
...
/* ============================================
   ...
   ============================================ */
function setFilterTanggalRiwayat() {
    filterBulanRiwayat = document.getElementById("filterBulanRiwayat").value;
    filterTahunRiwayat = document.getElementById("filterTahunRiwayat").value;
    renderTransaksi();
}

function setFilterTanggalGrafik() {
    filterBulanGrafik = document.getElementById("filterBulanGrafik").value;
    filterTahunGrafik = document.getElementById("filterTahunGrafik").value;
    renderGrafik();
}

function setFilterTanggalKategori() {
    filterBulanKategori = document.getElementById("filterBulanKategori").value;
    filterTahunKategori = document.getElementById("filterTahunKategori").value;
    renderKategoriPage();
}
...
function renderGrafik() {
    ...
    var bulanIni = transaksi.filter(function (t) {
        ...
        // Apply filter
        var matchBulan = filterBulanGrafik === "" || d.getMonth() == filterBulanGrafik;
        var matchTahun = filterTahunGrafik === "" || d.getFullYear() == filterTahunGrafik;
        return d.getMonth() === now.getMonth() && d.getFullYear() === now.getFullYear() && matchBulan && matchTahun;
    });
...
function renderMonthlyChart() {
    ...
    // Get all available months based on filter
    var filteredTrans = transaksi.filter(function(t) {
        var d = new Date(t.tanggal);
        var matchBulan = filterBulanGrafik === "" || d.getMonth() == filterBulanGrafik;
        var matchTahun = filterTahunGrafik === "" || d.getFullYear() == filterTahunGrafik;
    ...
    for (var i = 0; i < filteredTrans.length; i++) {
        var d = new Date(filteredTrans[i].tanggal);
        var key = d.getFullYear() + "-" + String(d.getMonth() + 1).padStart(2, "0");
        uniqueMonths[key] = { year: d.getFullYear(), month: d.getMonth() };
    }

    // Sort months chronologically (newest first)
    var sortedMonths = Object.keys(uniqueMonths).sort().reverse();

    // Limit to 6 months
    if (sortedMonths.length > 6) sortedMonths = sortedMonths.slice(0, 6);

    for (var i = sortedMonths.length - 1; i >= 0; i--) {
        var key = sortedMonths[i];
        var parts = key.split("-");
        var d = new Date(parseInt(parts[0]), parseInt(parts[1]) - 1, 1);
        var label = d.toLocaleDateString("id-ID", { month: "short", year: "numeric" });
        var masuk = 0, keluar = 0;

        for (var j = 0; j < filteredTrans.length; j++) {
            var td = new Date(filteredTrans[j].tanggal);
            var tkey = td.getFullYear() + "-" + String(td.getMonth() + 1).padStart(2, "0");
            if (tkey === key) {
                if (filteredTrans[j].jenis === "masuk") masuk += filteredTrans[j].jumlah;
                else keluar += filteredTrans[j].jumlah;
            }
        }
        months.push({ label: label, masuk: masuk, keluar: keluar });
    }

    // If no data, show current 6 months
    if (months.length === 0) {
        for (var i = 5; i >= 0; i--) {
            var d = new Date(now.getFullYear(), now.getMonth() - i, 1);
            var label = d.toLocaleDateString("id-ID", { month: "short", year: "2-digit" });
            months.push({ label: label, masuk: 0, keluar: 0 });
        }
    }
...
function renderDonutChart() {
    ...
    var pen = transaksi.filter(function(t) {
        ...
        var matchBulan = filterBulanGrafik === "" || d.getMonth() == filterBulanGrafik;
        var matchTahun = filterTahunGrafik === "" || d.getFullYear() == filterTahunGrafik;
        return t.jenis === "keluar" && matchBulan && matchTahun;
    });
...
function renderTransaksi() {
    ...
    // Filter tanggal
    if (filterBulanRiwayat !== "" || filterTahunRiwayat !== "") {
        data = data.filter(function(t) {
            var d = new Date(t.tanggal);
            var matchBulan = filterBulanRiwayat === "" || d.getMonth() == filterBulanRiwayat;
            var matchTahun = filterTahunRiwayat === "" || d.getFullYear() == filterTahunRiwayat;
            return matchBulan && matchTahun;
        });
    }
...
function renderKategoriPage() {
    ...
    var pen = transaksi.filter(function(t) {
        var d = new Date(t.tanggal);
        var matchBulan = filterBulanKategori === "" || d.getMonth() == filterBulanKategori;
        var matchTahun = filterTahunKategori === "" || d.getFullYear() == filterTahunKategori;
        return t.jenis === "keluar" && matchBulan && matchTahun;
    });
